...
{
	TWBR = 0x02; //2 calculated p.173 bit rate = 400.000 kbps, F_CPU = 8M   SCL freq= F_CPU/(16+2(TWBR).4^TWPS)
...
{
	TWAR = (1<<1);    // my address = 0x01 fel bit rakam wa7id msh 0
	TWCR = (1<<TWEN); //enable I2C
	return;
}
...
void i2c_repeated_start(void)
{
	...
	while((TWSR & 0xF8) != REP_START_ACK);              //masking f8 Check for the acknowledgment
	return;
}
...
void i2c_write_byte(unsigned char byte)
{
	...
	TWCR = (1<<TWINT) | (1<<TWEN);       // Clear TWI interrupt flag,Enable TWI
	while (!(TWCR & (1<<TWINT)));        // Wait till complete TWDR byte transmitted
	while((TWSR & 0xF8) != MSTR_WR_BYTE_ACK); // Check for the acknowledgment
	return;
}
...
unsigned char i2c_read_byte(void)
{
	TWCR = (1<<TWINT) | (1<<TWEN);             // Clear TWI interrupt flag,Enable TWI
	while(!(TWCR & (1<<TWINT)));               // Wait till complete TWDR byte transmitted
	while((TWSR & 0xF8) != MSTR_RD_BYTE_WITH_NACK); // Check for the acknowledgment
	return TWDR;
}
